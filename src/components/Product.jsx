...
const Product = ({product, isModal}) => {
    ...
    return (
        <article className={(isModal ? 'product--modal' : 'product')}>
            <picture className='product__image-box'>
                <img className='product__image' src={product.image} alt={product.name} />
            </picture>
            <div className={(isModal ? 'title-price--modal' : 'title-price')}>
                <h3 className='product__title'>{product.title}</h3> <p className='product__price'>${product.price}</p>
            </div>
            {!isModal && <p className='product__description'>{product.description}</p>}
            {!isModal && <button className='product__buy-button' onClick={() => addToCart(product)}>Comprar</button>}
        </article>
    );
};
